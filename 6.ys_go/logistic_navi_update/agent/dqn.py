...
class DQN(Agent):
    ...
    def run(self, num_episodes, max_step=None, buffer=2500, sampling=32,
            size_hidden=1, epoch=50, learning_rate=0.1, interval_train=10, run_time=2000, target_update=1000,
        ...
        for idx_epi in range(num_episodes):
            ...
            if idx_epi % interval_train == 0:
                if len(self.__replay_buffer) >= run_time:
                    loss_pre, loss_aft = self.__train(idx_epi, epoch, sampling, discount, target_update)
