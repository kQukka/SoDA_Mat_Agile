...
class Network:
    def __init__(self, input_size, output_size, h_size, learning_rate=0.1):
        ...
        self.w_2 = tf.Variable(tf.random.uniform([self.h_size, self.h_size], 0, 0.1), dtype=tf.float32)
        self.w_3 = tf.Variable(tf.random.uniform([self.h_size, self.output_size], 0, 0.1), dtype=tf.float32)

        self.optimizer = tf.optimizers.Adam(learning_rate=learning_rate)

    def predict(self, state):
        activation1 = tf.nn.tanh(tf.matmul(state, self.w_1))
        activation2 = tf.nn.tanh(tf.matmul(activation1, self.w_2))
        q_value = tf.matmul(activation2, self.w_3)
        return np.array(q_value.numpy())[0]

    def update(self, x_stack, y_stack):
        loss = lambda: tf.reduce_mean(
            input_tensor=tf.square(
                y_stack - tf.matmul(tf.nn.tanh(tf.matmul(tf.nn.tanh(tf.matmul(x_stack, self.w_1)), self.w_2)), self.w_3)))
        self.optimizer.minimize(loss, var_list=[self.w_1, self.w_2, self.w_3])
    ...
    def get_loss(self, x_stack, y_stack):
        self.loss = tf.reduce_mean(
            input_tensor=tf.square(y_stack - tf.matmul(tf.nn.tanh(tf.matmul(tf.nn.tanh(tf.matmul(x_stack, self.w_1)), self.w_2)), self.w_3)))
...
class DQN(Agent):
    ...
    def __init_run(self, max_step, size_hidden, buffer):
        ...
        self.dqn_target.w_3 = tf.Variable(tf.identity(self.dqn_update.w_3), dtype=tf.float32)
        ...
        while not done:
            ...
            if done:
                ...
                if reward == self.env.REWARD.GOAL:
                    ...
                    #ksy
                    while len(self.__temp_buffer) > 0:
