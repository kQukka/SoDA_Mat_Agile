...
def run_dqn():
    ...
    env.REWARD.GOAL = 150
    env.REWARD.OUT_GIRD = -100
    env.REWARD.OBSTACLE = -100
    env.REWARD.RETURN = -100
    env.REWARD.MOVE = -0.01

    q_map, reward = agent.run(10000,
                              buffer=10000, sampling=128,
                              size_hidden=24, epoch=10, learning_rate=0.001, interval_train=100,
                              discount=0.9, early_stopping=EarlyStopping(ratio=50), save_result=True,
                              greedy=0.999, run_time=2000, max_step=90)
