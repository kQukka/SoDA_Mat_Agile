...
class EarlyStopping:
    ...
    def check_stopping(self, value, min_step):
        self.__add_stack(value)
        num = self.__check_ratio(min_step)
        #print('--- check_stopping- num[{}] value[{}] min_step[{}]  cur_rate[{}] stack:[{}] '.format(
        #    num, value, min_step, ((num / self.patience) * 100), self.stack))
        if ((num / self.patience) * 100) >= self.ratio:
            return True
        return False

    def __add_stack(self, value):
        if len(self.stack) >= self.patience:  # ksy 11->10 if len(self.stack) > self.patience:
            del self.stack[0]
        self.stack.append(value)

    def __check_ratio(self, min_step):
        num = 0
        for val in self.stack:
            if val and val <= min_step:
                num += 1
        return num
