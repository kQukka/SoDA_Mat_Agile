...
class LogisticEnv(Env):
    """
    Logistic involves crossing a logistic map from Start(S) to Goal(G) without falling into any Holes(H) by walking over
    the Frozen(F) map. The agent may not always move in the intended direction due to the slippery nature of the logistic map.
